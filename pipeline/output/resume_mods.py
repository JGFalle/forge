...
def generate(
    ...
    story = _build_story(
        jd, company, role, tailoring_data, salary_intel, styles, fonts, today_str,
        exec_intel_result=exec_intel_result or {},
    )
    doc.build(story, onFirstPage=_footer, onLaterPages=_footer)
    logger.info("Resume modifications PDF saved: %s", out_path)
    return out_path
...
def _build_story(
    ...
    # Title
    story.append(Paragraph("RESUME MODIFICATIONS", styles["eyebrow"]))
    story.append(Paragraph(html.escape(company), styles["h1"]))
    story.append(Paragraph(html.escape(role), styles["role"]))
    story.append(Spacer(1, 10))

    # Compensation
    if salary_intel or (jd.salary_range and jd.salary_range not in ("Not listed", "Not specified")):
    ...
    if agg and not agg.get("raw"):  # only show if aggregation succeeded
        ...
        if talking_points:
            ...
            for pt in talking_points:
                story.append(Paragraph(f"&bull;&nbsp;&nbsp;{html.escape(_strip_md(pt))}", styles["bullet"]))
...
def _distill_why(company_strategy: str, role_context: str) -> str:
    ...
    parts = []
    if company_strategy:
        parts.append(first_sentence(company_strategy))
    if role_context:
        parts.append(first_sentence(role_context))
