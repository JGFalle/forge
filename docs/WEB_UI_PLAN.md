...
- **Secrets:** API keys move from `.env` to per-user settings (or the host's env
  in Tier 2). OSS mode means a friend with *no* keys still gets full output —
  keep that the default so onboarding is "upload resume, drop JD."
- **Per-user config isolation:** today `config/config.yaml` is global. Web needs
  one config per user. Refactor `utils/config.py` to accept an override path /
  in-memory dict (small change; keeps the dot-path `get()` API). This is the main
